...
export function businessMetrics(jobs: Job[], invoices: Invoice[], leads: Lead[], expenses: Expense[], crew: CrewMember[], targetDate = isoToday()) {
  ...
  const projectedRevenue = revenueJobs
    .filter((job) => job.status !== "completed" && job.date >= targetDate)
    .reduce((sum, job) => sum + job.price, 0);
  const completedRevenue = revenueJobs
  ...
  return {
    ...
    totalTips,
    upcomingJobs: jobs.filter((job) => isUpcomingJob(job, targetDate)).length,
