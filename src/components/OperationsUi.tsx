...
import { BriefcaseBusiness, Check, Mail, Pencil, Phone, Plus, Search, X } from "lucide-react";
import { currency } from "../lib/calculations";
import type { Customer, Job, JobCreateInput, Lead, LeadStatus, PaymentStatus, PlanType } from "../types/business";
...
const recurringFrequencies: Array<{ value: PlanType; label: string; months: number }> = [
  { value: "monthly", label: "Monthly", months: 1 },
  { value: "3-month", label: "Every 3 months", months: 3 },
  { value: "4-month", label: "Every 4 months", months: 4 },
  { value: "6-month", label: "Every 6 months", months: 6 },
  { value: "yearly", label: "Yearly", months: 12 },
];

function renewalDateFrom(date: string, frequency: PlanType) {
  const months = recurringFrequencies.find((item) => item.value === frequency)?.months ?? 1;
...
export function CreateRecordModal({ kind, customers, currentDate, onClose, onCreateCustomer, onCreateJob, onCreateLead }: {
  ...
  currentDate: string;
  onClose: () => void;
  onCreateCustomer: (draft: Omit<Customer, "id" | "insights" | "subscribedPlanId" | "websiteEditedFields">) => Promise<Customer>;
  onCreateJob: (draft: JobCreateInput) => Promise<void>;
  ...
  const [job, setJob] = useState({ date: currentDate, time: "09:00", customerId: "", address: "", serviceType: "", status: "scheduled" as Job["status"], price: 0, notes: "", employeeInstructions: "" });
  const [recurring, setRecurring] = useState(false);
  const [recurrence, setRecurrence] = useState({ frequency: "3-month" as PlanType, renewalDate: renewalDateFrom(currentDate, "3-month"), paymentStatus: "unpaid" as PaymentStatus });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (kind === "job") {
        const normalizedName = jobCustomerName.trim().toLowerCase();
        const existingCustomer = customers.find((item) => item.name.trim().toLowerCase() === normalizedName);
        const customerId = existingCustomer?.id ?? (await onCreateCustomer({ name: jobCustomerName.trim(), phone: "", email: "", address: job.address, notes: "" })).id;
        await onCreateJob({ ...job, customerId, price: Number(jobPrice) || 0, recurrence: recurring ? recurrence : undefined });
      }
  ...
  return <ModalShell title={title} kicker="Create record" onClose={onClose}><form onSubmit={submit}><div className="settings-grid mt-5">{kind === "customer" && <><label>Name<input required value={customer.name} onChange={(event) => setCustomer({ ...customer, name: event.target.value })} /></label><label>Phone<input value={customer.phone} onChange={(event) => setCustomer({ ...customer, phone: event.target.value })} /></label><label>Email<input type="email" value={customer.email} onChange={(event) => setCustomer({ ...customer, email: event.target.value })} /></label><label>Address<input value={customer.address} onChange={(event) => setCustomer({ ...customer, address: event.target.value })} /></label><label className="sm:col-span-2">Notes<textarea value={customer.notes} onChange={(event) => setCustomer({ ...customer, notes: event.target.value })} /></label></>}{kind === "job" && <><label>Job type<select value={recurring ? "recurring" : "one-time"} onChange={(event) => setRecurring(event.target.value === "recurring")}><option value="one-time">One-time job</option><option value="recurring">Recurring job</option></select></label><label>Status<select value={job.status} onChange={(event) => setJob({ ...job, status: event.target.value as Job["status"] })}>{jobStatuses.map((status) => <option key={status}>{status}</option>)}</select></label><label>Customer<input list="job-customer-options" required value={jobCustomerName} placeholder="Type customer name" onChange={(event) => { const name = event.target.value; const selected = customers.find((item) => item.name.trim().toLowerCase() === name.trim().toLowerCase()); setJobCustomerName(name); setJob((current) => { const previous = customers.find((item) => item.id === current.customerId); const addressWasAutomatic = !current.address || current.address === previous?.address; return { ...current, customerId: selected?.id ?? "", address: selected?.address || (addressWasAutomatic ? "" : current.address) }; }); }} /><datalist id="job-customer-options">{customers.map((item) => <option key={item.id} value={item.name} />)}</datalist></label><label>Date<input type="date" required value={job.date} onChange={(event) => { const date = event.target.value; setJob({ ...job, date }); if (recurring) setRecurrence((current) => ({ ...current, renewalDate: renewalDateFrom(date, current.frequency) })); }} /></label><label>Time<input type="time" required value={job.time} onChange={(event) => setJob({ ...job, time: event.target.value })} /></label><label>Service<input required value={job.serviceType} onChange={(event) => setJob({ ...job, serviceType: event.target.value })} /></label><label>Price<input type="number" min="0" step="0.01" value={jobPrice} onChange={(event) => setJobPrice(event.target.value)} /></label><label className="sm:col-span-2">Address<input required value={job.address} onChange={(event) => setJob({ ...job, address: event.target.value })} /></label>{recurring && <><label>Frequency<select value={recurrence.frequency} onChange={(event) => { const frequency = event.target.value as PlanType; setRecurrence((current) => ({ ...current, frequency, renewalDate: renewalDateFrom(job.date, frequency) })); }}>{recurringFrequencies.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>Next service date<input type="date" required value={recurrence.renewalDate} onChange={(event) => setRecurrence({ ...recurrence, renewalDate: event.target.value })} /></label><label>Plan payment status<select value={recurrence.paymentStatus} onChange={(event) => setRecurrence({ ...recurrence, paymentStatus: event.target.value as PaymentStatus })}><option value="unpaid">Unpaid</option><option value="paid">Paid</option><option value="partially paid">Partially paid</option><option value="past due">Past due</option></select></label><div className="rounded-lg border border-cyan-100 bg-mist p-3 text-sm font-normal text-lagoon dark:border-cyan-500/20 dark:bg-cyan-500/10 dark:text-cyan-200">This creates the scheduled job plus a linked service plan in the Recurring Jobs sheet.</div></>}<label className="sm:col-span-2">Instructions for employee<textarea value={job.employeeInstructions} onChange={(event) => setJob({ ...job, employeeInstructions: event.target.value })} placeholder="Gate code, water access, pets, surfaces to avoid, customer requests, or equipment needed" /></label><label className="sm:col-span-2">Internal job notes<textarea value={job.notes} onChange={(event) => setJob({ ...job, notes: event.target.value })} /></label></>}{kind === "lead" && <><label>Name<input required value={lead.name} onChange={(event) => setLead({ ...lead, name: event.target.value })} /></label><label>Status<select value={lead.status} onChange={(event) => setLead({ ...lead, status: event.target.value as LeadStatus })}>{leadStatuses.map((status) => <option key={status}>{status}</option>)}</select></label><label>Contact information<input value={lead.contact} onChange={(event) => setLead({ ...lead, contact: event.target.value })} /></label><label>Estimated value<input type="number" min="0" step="0.01" value={lead.estimatedValue} onChange={(event) => setLead({ ...lead, estimatedValue: Number(event.target.value) || 0 })} /></label><label>Follow-up date<input type="date" value={lead.followUpDate} onChange={(event) => setLead({ ...lead, followUpDate: event.target.value })} /></label><label>Address<input value={lead.address} onChange={(event) => setLead({ ...lead, address: event.target.value })} /></label><label className="sm:col-span-2">Notes<textarea value={lead.notes} onChange={(event) => setLead({ ...lead, notes: event.target.value })} /></label></>}</div>{error && <p className="mt-4 rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-200">{error}</p>}<FormActions saving={saving} onClose={onClose} /></form></ModalShell>;
}
...
export function GlobalSearch({ customers, jobs, onJob, onNew }: { customers: Customer[]; jobs: Job[]; onJob: (job: Job) => void; onNew: (kind: CreateKind) => void }) {
  ...
  const results = useMemo(() => {
    ...
    return jobs.filter((item) => [customerNames.get(item.customerId) ?? "Unknown customer", item.address, item.serviceType, item.date, item.status].join(" ").toLowerCase().includes(normalized)).slice(0, 10).map((item) => ({ item, title: customerNames.get(item.customerId) ?? "Unknown customer", detail: `${item.date} · ${item.serviceType}` }));
  }, [customerNames, jobs, normalized]);
  function choose(job: Job) { setQuery(""); onJob(job); }
  return <div className="relative border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900"><div className="mx-auto flex max-w-[1500px] gap-2"><div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={17} /><input aria-label="Search jobs" className="h-10 w-full rounded-lg border border-slate-200 bg-slate-50 pl-10 pr-3 text-sm outline-none focus:border-lagoon dark:border-slate-700 dark:bg-slate-950" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search jobs by customer, address, service, date, or status" /></div><div className="relative"><button type="button" className="primary-button h-10 gap-2" aria-label="Create new record" title="Create new record" onClick={() => { setQuery(""); setNewOpen(!newOpen); }}><Plus size={16} /><span className="hidden sm:inline">New</span></button>{newOpen && <div className="absolute right-0 top-12 z-40 w-48 rounded-lg border border-slate-200 bg-white p-1 shadow-soft dark:border-slate-700 dark:bg-slate-900">{(["customer", "job", "lead"] as CreateKind[]).map((item) => <button key={item} type="button" className="nav-item capitalize" onClick={() => { setNewOpen(false); onNew(item); }}><Plus size={15} />{item}</button>)}</div>}</div></div>{normalized && <div className="absolute left-4 right-4 top-[62px] z-30 mx-auto max-h-80 max-w-[1500px] overflow-auto rounded-lg border border-slate-200 bg-white p-2 shadow-soft dark:border-slate-700 dark:bg-slate-900">{results.map((result) => <button key={result.item.id} type="button" className="flex w-full items-center gap-3 rounded-lg p-3 text-left hover:bg-slate-50 dark:hover:bg-slate-800" onClick={() => choose(result.item)}><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-mist text-lagoon"><BriefcaseBusiness size={17} /></span><span className="min-w-0"><strong className="block truncate text-sm text-ink dark:text-white">{result.title}</strong><span className="block truncate text-xs text-slate-500">{result.detail}</span></span></button>)}{results.length === 0 && <p className="p-4 text-center text-sm text-slate-500">No matching jobs</p>}</div>}</div>;
}
