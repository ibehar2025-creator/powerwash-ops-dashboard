...
import type { ContractSubmission, EarningSubmission, Job, JobAssignment, JobStatus, PayrollRun } from "../types/business";

type EmployeeTab = "home" | "schedule" | "map" | "earnings" | "contracts";
...
function EmployeeHome({ data, jobsToday, assignmentMap, customerMap, onJob }: { data: EmployeeWorkspaceSnapshot; jobsToday: Job[]; assignmentMap: Map<string, unknown>; customerMap: Map<string, EmployeeWorkspaceSnapshot["customers"][number]>; onJob: (job: Job) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800"><div><p className="text-xs font-semibold uppercase tracking-wide text-lagoon">Today and upcoming</p><h3 className="mt-1 text-xl font-bold text-ink dark:text-white">Assigned jobs</h3></div><span className="grid h-10 w-10 place-items-center rounded-xl bg-cyan-50 text-lagoon dark:bg-cyan-500/10 dark:text-cyan-200"><MapPinned size={19} /></span></div><div className="grid gap-3 p-4 lg:grid-cols-2">{upcoming.map((job, index) => { const customer = customerMap.get(job.customerId); return <button key={job.id} className="group rounded-xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-4 text-left transition hover:-translate-y-0.5 hover:border-lagoon hover:shadow-md dark:border-slate-700 dark:from-slate-900 dark:to-slate-900/60" onClick={() => onJob(job)}><div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-lagoon text-sm font-bold text-white shadow-sm">{index + 1}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-lagoon">{job.date === today ? "Today" : formatScheduleDate(job.date, { month: "short", day: "numeric" })} · {job.time}</p><strong className="mt-1 block text-lg text-ink dark:text-white">{customer?.name ?? "Customer"}</strong></div><span className={`rounded-md px-2 py-1 text-xs font-semibold capitalize ${statusStyle(job.status)}`}>{job.status}</span></div><p className="mt-2 text-sm text-slate-500">{job.address}</p><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-md bg-slate-100 px-2 py-1 text-slate-600 dark:bg-slate-800 dark:text-slate-300">{job.serviceType}</span><span className="rounded-md bg-emerald-50 px-2 py-1 font-semibold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-200">{currency.format(job.price)}</span></div></div></div></button>; })}{!upcoming.length && <div className="col-span-full rounded-xl border border-dashed border-cyan-200 bg-cyan-50/50 px-5 py-10 text-center dark:border-cyan-500/20 dark:bg-cyan-500/5"><CalendarDays className="mx-auto text-lagoon dark:text-cyan-200" size={30} /><p className="mt-3 font-semibold text-ink dark:text-white">No assigned jobs</p><p className="mt-1 text-sm text-slate-500">New assigned work will appear here.</p></div>}</div></section>
  </div>;
}
...
function EarningsModal({ submitEarnings = submitEmployeeEarnings, job, employeeId, assignment, contracts, existing, onOpenContracts, onClose, onSaved }: { submitEarnings?: typeof submitEmployeeEarnings; job: Job; employeeId?: string; assignment?: JobAssignment; contracts: ContractSubmission[]; existing?: EarningSubmission; onOpenContracts: () => void; onClose: () => void; onSaved: (saved: EarningSubmission) => void }) {
  ...
  const [contractSubmissionId, setContractSubmissionId] = useState(existing?.contractSubmissionId ?? contracts[0]?.id ?? "");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const saved = await submitEarnings({
        ...
        contractSubmissionId: hasContract ? contractSubmissionId : undefined,
        employeeId,
        hasUpsell,
  ...
  const basePct = assignment?.baseCommissionPct ?? 0;
  const contractPct = assignment?.contractBonusPct ?? 0.1;
  const combinedPct = basePct + (hasContract && contractSubmissionId ? contractPct : 0);
  const jobCommission = job.price * combinedPct;
  return <div className="fixed inset-0 z-[90] grid place-items-center bg-ink/55 p-3"><form className="max-h-[94dvh] w-full max-w-lg min-w-0 overflow-auto rounded-xl bg-white p-5 shadow-soft dark:bg-slate-900" onSubmit={submit}><div className="flex justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase text-lagoon">Job compensation</p><h2 className="text-xl font-bold text-ink dark:text-white">Earnings, tip, upsell &amp; contract</h2></div><button type="button" className="icon-button shrink-0" onClick={onClose}><X size={17} /></button></div><p className="mt-2 text-sm text-slate-500">Submit everything from this job in one place for owner approval. This also marks the job completed.</p><div className="mt-5 min-w-0 space-y-5"><label className="block min-w-0 text-sm font-semibold"><span className="block">Customer tip</span><input className={employeeInputClass} type="number" min="0" step="0.01" inputMode="decimal" value={tip} onChange={(event) => setTip(event.target.value)} placeholder="0.00" /></label><label className="block min-w-0 text-sm font-semibold"><span className="block">Gas reimbursement ($)</span><input className={employeeInputClass} type="number" min="0" max="999999.99" step="0.01" inputMode="decimal" value={gasCost} onChange={(event) => setGasCost(event.target.value)} placeholder="0.00" /><span className="mt-2 block text-xs font-normal text-slate-500">Gas cost for this job, reimbursed in full after owner approval.</span></label><fieldset><legend className="text-sm font-semibold">Did you get this customer on a contract?</legend><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" className={hasContract ? "primary-button" : "text-button"} aria-pressed={hasContract} onClick={() => setHasContract(true)}>Yes</button><button type="button" className={!hasContract ? "primary-button" : "text-button"} aria-pressed={!hasContract} onClick={() => setHasContract(false)}>No</button></div></fieldset>{hasContract && <div className="space-y-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-500/25 dark:bg-emerald-500/10">{contracts.length ? <><label className="block text-sm font-semibold">Signed customer agreement<select className={employeeInputClass} required value={contractSubmissionId} onChange={(event) => setContractSubmissionId(event.target.value)}>{contracts.map((contract) => <option key={contract.id} value={contract.id}>{contract.customerName} · {contract.frequency} · {contract.status}</option>)}</select></label><p className="text-sm font-semibold text-emerald-800 dark:text-emerald-200">Contract bonus: {Math.round(basePct * 100)}% + {Math.round(contractPct * 100)}% = {Math.round(combinedPct * 100)}% of the entire {currency.format(job.price)} job ({currency.format(jobCommission)} before tips and upsells).</p><p className="text-xs text-emerald-700 dark:text-emerald-300">The owner must approve both the earnings and the signed contract.</p></> : <><p className="text-sm font-semibold text-emerald-800 dark:text-emerald-200">A signed agreement must be linked to this job first.</p><button type="button" className="text-button w-full gap-2" onClick={onOpenContracts}><FileSignature size={16} />Create and sign contract</button></>}</div>}<fieldset><legend className="text-sm font-semibold">Did you offer an upsell?</legend><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" className={hasUpsell ? "primary-button" : "text-button"} aria-pressed={hasUpsell} onClick={() => setHasUpsell(true)}>Yes</button><button type="button" className={!hasUpsell ? "primary-button" : "text-button"} aria-pressed={!hasUpsell} onClick={() => setHasUpsell(false)}>No</button></div></fieldset>{hasUpsell && <div className="space-y-4 rounded-xl border border-cyan-100 bg-cyan-50/60 p-4 dark:border-cyan-500/20 dark:bg-cyan-500/10"><div className="flex items-center gap-2 text-lagoon dark:text-cyan-200"><Sparkles size={17} /><p className="text-sm font-semibold">Upsell details</p></div><label className="block min-w-0 text-sm font-semibold"><span className="block">Service offered</span><input className={employeeInputClass} required value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Back patio cleaning" /></label><label className="block min-w-0 text-sm font-semibold"><span className="block">Quoted price</span><input className={employeeInputClass} required type="number" min="0" step="0.01" inputMode="decimal" value={quotedAmount} onChange={(event) => setQuotedAmount(event.target.value)} placeholder="125.00" /></label><label className="block min-w-0 text-sm font-semibold"><span className="block">Customer result</span><select className={employeeInputClass} value={outcome} onChange={(event) => setOutcome(event.target.value as typeof outcome)}><option value="accepted">Accepted</option><option value="declined">Declined</option><option value="follow-up">Follow up later</option></select></label><label className="block min-w-0 text-sm font-semibold"><span className="block">Conversation notes</span><textarea className={employeeTextareaClass} value={upsellNotes} onChange={(event) => setUpsellNotes(event.target.value)} placeholder="What the customer said, timing, or follow-up details" /></label></div>}</div>{error && <p className="mt-4 rounded-lg bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}<button className="primary-button mt-5 w-full gap-2" disabled={saving || (hasContract && !contractSubmissionId)}><DollarSign size={16} />{saving ? "Submitting..." : "Submit earnings & mark completed"}</button></form></div>;
}

function EarningsReceipt({ earning }: { earning: EarningSubmission }) {
  const lines = [
    { label: "Base commission", amount: earning.baseEarnings },
    { label: "Contract bonus", amount: earning.contractEarnings },
    { label: "Upsell commission", amount: earning.upsellEarnings },
    { label: "Tip", amount: earning.tipEarnings },
    { label: "Gas reimbursement", amount: earning.gasCost ?? 0 },
  ];
