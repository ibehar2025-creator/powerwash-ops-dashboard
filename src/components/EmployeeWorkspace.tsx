...
function EarningsModal({ job, employeeId, existing, onClose, onSaved }: { job: Job; employeeId?: string; existing?: EarningSubmission; onClose: () => void; onSaved: (saved: EarningSubmission) => void }) {
  ...
  return <div className="fixed inset-0 z-[90] grid place-items-center bg-ink/55 p-3"><form className="w-full max-w-md min-w-0 rounded-lg bg-white p-5 dark:bg-slate-900" onSubmit={submit}><div className="flex justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase text-lagoon">Job compensation</p><h2 className="text-xl font-bold text-ink dark:text-white">Submit earnings details</h2></div><button type="button" className="icon-button shrink-0" onClick={onClose}><X size={17} /></button></div><p className="mt-2 text-sm text-slate-500">Base commission and accepted upsell commission are calculated automatically. Enter the customer tip here.</p><div className="mt-5 min-w-0 space-y-4"><label className="block min-w-0 text-sm font-semibold"><span className="block">Customer tip</span><input className={employeeInputClass} type="number" min="0" step="0.01" inputMode="decimal" value={tip} onChange={(event) => setTip(event.target.value)} placeholder="0.00" /></label>{existing?.upsellDescription && <div className="rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-800"><p className="font-semibold">Upsell: {existing.upsellDescription}</p><p className="mt-1 text-xs text-slate-500">{existing.upsellOutcome} · quoted {currency.format(existing.upsellQuotedAmount)}</p></div>}</div>{error && <p className="mt-4 rounded-lg bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}<button className="primary-button mt-5 w-full" disabled={saving}>{saving ? "Submitting..." : "Send for owner approval"}</button></form></div>;
}
