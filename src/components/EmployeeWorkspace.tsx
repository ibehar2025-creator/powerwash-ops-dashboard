...
export function EmployeeWorkspace({ preview, onExitPreview }: { preview?: boolean; onExitPreview?: () => void }) {
  ...
  return <div className={darkMode ? "dark" : ""}><div className="flex min-h-screen bg-slate-100 text-slate-700 dark:bg-slate-950 dark:text-slate-200">
    ...
    {earningsJob && <EarningsModal job={earningsJob} employeeId={preview ? employee?.id : undefined} assignment={assignmentMap.get(earningsJob.id)} contracts={(data?.contracts ?? []).filter((contract) => contract.jobId === earningsJob.id && contract.status !== "rejected")} existing={data?.earnings.find((item) => item.jobId === earningsJob.id)} onOpenContracts={() => { setEarningsJob(null); setActiveTab("contracts"); }} onClose={() => setEarningsJob(null)} onSaved={(saved) => { setData((current) => current ? { ...current, earnings: current.earnings.some((item) => item.id === saved.id) ? current.earnings.map((item) => item.id === saved.id ? saved : item) : [saved, ...current.earnings] } : current); setEarningsJob(null); }} />}
  </div></div>;
}
