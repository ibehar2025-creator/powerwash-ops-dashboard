  ...
  return (
    <section className="mx-auto w-full max-w-[1500px] rounded-lg border border-slate-200 bg-white shadow-soft dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="jobs-sheet-mobile divide-y divide-slate-200 md:hidden dark:divide-slate-800">
        {rows.map((job, index) => { const sheetRow = rowNumber(job, index); const status = spreadsheetStatus(job); return <button key={job.id} type="button" className="block w-full p-4 text-left transition hover:bg-slate-50 dark:hover:bg-slate-800/60" onClick={() => onEditJob(job)}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold text-slate-400">Row {sheetRow}</p><p className="truncate font-semibold text-ink dark:text-white">{customerName(job, customers) || "Blank"}</p><p className="mt-1 text-sm text-slate-500">{job.address || "No address"}</p></div><Pencil className="shrink-0 text-slate-400" size={16} /></div><dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 text-sm"><div><dt>Date</dt><dd>{originalDate(job)}</dd></div><div><dt>Price</dt><dd>{job.price ? currency.format(job.price) : ""}</dd></div><div><dt>Status</dt><dd><span className={statusClass(status)}>{status}</span></dd></div><div><dt>Notes</dt><dd>{job.serviceType}</dd></div></dl></button>; })}
      </div>
    </section>
  );
}
