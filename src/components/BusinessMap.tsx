    ...
    <div className="min-w-0 space-y-4 overflow-x-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="text-xl font-bold text-ink dark:text-white">{employeeView ? "Assigned job map" : "Jobs and canvassing map"}</h2>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{employeeView ? "Only jobs assigned to you are shown." : "Click a property to record a door you solicited."}</p>
          {!employeeView && <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Every job has its own marker. Zoom in to separate nearby properties.</p>}
        </div>
        <div className="flex flex-wrap items-center gap-3 text-sm">
          <label className="inline-flex items-center gap-2 font-medium text-slate-600 dark:text-slate-300">
            ...
            <span className="h-2.5 w-2.5 rounded-full bg-emerald-600" /> {employeeView ? "Assigned jobs" : "All jobs"}
          </label>
          {!employeeView && <label className="inline-flex items-center gap-2 font-medium text-slate-600 dark:text-slate-300">
      ...
      <div className={`grid min-h-0 min-w-0 gap-4 ${employeeView ? "" : "xl:grid-cols-[minmax(0,1fr)_360px]"}`}>
        <div className="relative h-[58vh] min-h-[480px] min-w-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900 xl:h-[calc(100vh-245px)] xl:max-h-[760px]">
          <Map
            ...
            reuseMaps
            onClick={employeeView ? undefined : handleMapClick}
          >
            <MapFocusController request={mapFocusRequest} />
            {showJobs && <JobMarkers locations={jobLocations} onSelect={selectJobLocation} />}
            {!employeeView && showSolicitations && <SolicitationMarkers locations={visibleSolicitations} onSelect={selectSolicitation} />}
            {!employeeView && draftCoordinates && (
              <Marker
      ...
      {!employeeView && <div className="grid gap-3 sm:grid-cols-3">
