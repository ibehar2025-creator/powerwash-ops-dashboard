...
function OwnerDashboard({ onPreviewEmployee }: { onPreviewEmployee: () => void }) {
  ...
  useEffect(() => {
    void syncSheets();
    const interval = window.setInterval(() => void syncSheets(), 14 * 60_000);
    return () => window.clearInterval(interval);
  }, [syncSheets]);
...
function JobModal({ customers, job, onSave, onSaveCustomer, onDelete, onFindOnMap, onClose }: { customers: Customer[]; job: Job; onSave: (jobId: string, patch: Partial<Job>) => Promise<Job>; onSaveCustomer: (customerId: string, patch: Partial<Customer>) => Promise<void>; onDelete: (jobId: string) => Promise<void>; onFindOnMap: (job: Job) => void; onClose: () => void }) {
  ...
  const [phoneInput, setPhoneInput] = useState(findCustomer(customers, job.customerId).phone ?? "");
  const [saving, setSaving] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [confirmingDelete, setConfirmingDelete] = useState(false);
  const [error, setError] = useState("");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const selectedCustomer = findCustomer(customers, draft.customerId);
    const phoneChanged = phoneInput.trim() !== (selectedCustomer.phone ?? "");
    try {
      if (phoneChanged) {
        await onSaveCustomer(draft.customerId, { phone: phoneInput.trim() });
