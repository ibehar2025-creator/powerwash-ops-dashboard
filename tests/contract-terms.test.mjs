...
const builder = source.slice(source.indexOf('function annualCleaningCount('), source.indexOf('export function EmployeeContractFlow'));
...
test('new recurring agreements include every visible requested term', () => {
  for (const frequency of ['Every 6 months', 'Monthly', 'Yearly']) {
    ...
    for (const clause of ["at least 24 hours' notice", 'rain, storms, freezing temperatures, or unsafe working conditions', 'automated text messages and emails', 'appointment reminders, schedule updates, invoice notifications, and plan renewal reminders', "Company's CRM system", 'over a 12-month period', 'does not automatically renew', 'agree in writing to continue on new terms', 'By signing electronically, Customer confirms']) assert.ok(text.includes(clause), clause);
...
test('signing submits the displayed agreement snapshot', () => {
  ...
  assert.match(source, /\{recurringTerms\(draft.frequency\)\}<\/div>/);
});

test('cleaning counts follow recognized frequencies without guessing custom schedules', () => {
  for (const [frequency, count] of [['Monthly', 12], ['Quarterly', 4], ['Every 4 months', 3], ['6-month', 2], ['Yearly', 1]]) {
    assert.equal(context.annualCleaningCount(frequency), count);
    assert.ok(context.recurringTerms(frequency).includes(count + ' scheduled cleaning'));
  }
  assert.equal(context.annualCleaningCount('As needed'), null);
  assert.ok(context.recurringTerms('As needed').includes('agreed frequency of "As needed"'));
});
